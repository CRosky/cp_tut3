...
def simulate_lorentzian(t,a=1,b=1,cent=0):
    dat=a/(b+(t-cent)**2)
    dat+=numpy.random.randn(t.size)
    return dat
...
class Lorentzian:
    def __init__(self,t,a=1,b=1,cent=0,offset=0):
          self.t=t
          self.y=simulate_lorentzian(t,a,b,cent)+offset
          self.err=numpy.ones(t.size)
          self.a=a
          self.b=b
          self.cent=cent
...
if __name__=='__main__':
    ...
    param_true=numpy.array([dat_lor.a,dat_lor.b,dat_lor.cent,dat_lor.offset])
    for i in range(0,param_true.size):
        val=numpy.mean(chain[:,i])
        scat=numpy.std(chain[:,i])
        print [param_true[i],val,scat]
